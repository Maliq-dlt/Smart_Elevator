...
import { IndustrialSimulatorView } from './components/IndustrialSimulatorView';
...
import { ShieldAlert, CheckCircle, XCircle, Gauge } from 'lucide-react';
...
export default function App() {
    ...
    const lastAutoSpawnTime = useRef(0);
    const [isDebug, setIsDebug] = useState(false); // DEBUG STATE
    const [industrialView, setIndustrialView] = useState(false); // INDUSTRIAL SIMULATOR VIEW
    ...
    const handleApiInject = (data: any) => {
        if (data.lift && (data.lift === 'A' || data.lift === 'B') && data.target) {
            const targetFloor = Number(data.target);
            if (targetFloor >= 1 && targetFloor <= 3) {
                const setter = data.lift === 'A' ? setLiftA : setLiftB;
                setter(prev => {
                    // Force inject command
                    if (prev.status === 'IDLE' || prev.status === 'MOVING') {
                        addLog(`API INJECTION: Force Lift ${data.lift} to Floor ${targetFloor}`);
                        return {
                            ...prev,
                            status: 'MOVING',
    ...
    const generateReport = async () => {
        ...
        setAnalysisReport("Sedang menganalisis performa sistem secara komprehensif...");
        const report = await generateScenarioAnalysis(history, stats);
    ...
    useEffect(() => {
        ...
        const interval = setInterval(() => {
            setSimTime(prev => {
                // Apply timeScale to simulation time advancement
                const next = prev + (TICK_RATE / 1000) * timeScale;
    ...
    }, [isRunning, step, systemMode, liftA, liftB, building, pendingPassengers, activeScenario, bootSequence, fireFloor, snapTarget, powerScope, approvalRequests, isAutoSimulating, timeScale]);
    ...
    const updateSystem = () => {
        ...
        // --- AUTO SIMULATION LOGIC INTEGRATED ---
        if (isAutoSimulating && (currentSimTime - lastAutoSpawnTime.current > 3.0)) {
            const totalActivePassengers =
                pendingPassengers.length +
                liftA.passengers.length +
                liftB.passengers.length +
                Object.values(building.floors).reduce((sum: number, f: any) => sum + f.waitingPassengers.length, 0);

            const MAX_AUTO_PASSENGERS = 8;

            if (totalActivePassengers < MAX_AUTO_PASSENGERS) {
                const start = Math.floor(Math.random() * 3) + 1;
                let dest = Math.floor(Math.random() * 3) + 1;
                while (dest === start) dest = Math.floor(Math.random() * 3) + 1;

                const newPax: PassengerConfig = {
                    id: `auto-${Date.now()}-${Math.random().toFixed(4)}`,
                    name: MOCK_NAMES[Math.floor(Math.random() * MOCK_NAMES.length)],
                    weight: Math.floor(Math.random() * 50) + 50,
                    startFloor: start,
                    destinationFloor: dest,
                    requestTime: currentSimTime
                };

                setPendingPassengers(prev => [...prev, newPax]);
                addLog(`AUTO SIM: ${newPax.name} generated at Floor ${start}.`, 'INFO');
                lastAutoSpawnTime.current = currentSimTime;
            }
        }
        ...
        const chargeBattery = (lift: LiftState, hasPower: boolean) => {
            ...
            if (hasPower && newLevel < 100) newLevel += 0.2 * timeScale;
        ...
        const resultA = processLiftTick(
            ...
            snapTarget === 'A' || snapTarget === 'BOTH', snapTimerA, approvalRequests, currentSimTime, timeScale
        );
        ...
        const resultB = processLiftTick(
            ...
            snapTarget === 'B' || snapTarget === 'BOTH', snapTimerB, approvalRequests, currentSimTime, timeScale
        );
    ...
    return (
        <div className="min-h-screen bg-slate-950 text-slate-200 font-sans relative">
            ...
            {/* Industrial View Mode */}
            {industrialView ? (
                <div className="h-screen flex flex-col">
                    {/* Toggle back button */}
                    <button
                        onClick={() => setIndustrialView(false)}
                        className="absolute top-4 left-4 z-50 px-4 py-2 bg-slate-800 hover:bg-slate-700 rounded-lg text-sm font-bold border border-slate-600 flex items-center gap-2"
                    >
                        ← Classic View
                    </button>
                    <IndustrialSimulatorView
                        liftA={liftA}
                        liftB={liftB}
                        isRunning={isRunning}
                        timeScale={timeScale}
                ...
                <div className="p-4 md:p-8">
                    <Header
                        ...
                        timeScale={timeScale} setTimeScale={setTimeScale} isDebug={isDebug} setIsDebug={setIsDebug}
                    />

                    {/* Industrial View Toggle Button */}
                    <div className="mb-4 flex justify-end">
                        <button
                            onClick={() => setIndustrialView(true)}
                            className="px-4 py-2 bg-gradient-to-r from-cyan-600 to-purple-600 hover:from-cyan-500 hover:to-purple-500 rounded-lg text-sm font-bold flex items-center gap-2 shadow-lg shadow-cyan-900/30"
                        >
                            <Gauge size={18} />
                            Industrial Telemetry View
                        </button>
                    </div>

                    <main className="grid grid-cols-1 lg:grid-cols-12 gap-6">
                        <div className="lg:col-span-7 space-y-6">
                            <BuildingVisualizer liftA={liftA} liftB={liftB} building={building} mode={systemMode} fireFloor={fireFloor} isDebug={isDebug} />
