...
/**
 * Result of a single simulation tick for one elevator.
 * Contains the updated lift state, building state, statistics, and any logs generated.
 */
export interface TickResult {
    /** Updated elevator state after this tick */
    lift: LiftState;
    /** Updated building state (waiting passengers, etc.) */
    building: BuildingState;
    /** Updated simulation statistics */
    stats: SimulationStats;
    /** Log messages generated during this tick */
    logs: string[];
    /** Optional approval request if user intervention is needed */
...
/**
 * Creates an initial lift state for a new elevator.
 * 
 * @param id - Unique identifier for the lift ('A' or 'B')
 * @param startFloor - The floor where the lift starts (1-based)
 * @returns A complete initial LiftState object with sensors, safety, and panel initialized
 * 
 * @example
 * const liftA = initialLiftState('A', 1); // Lift A starting at floor 1
 * const liftB = initialLiftState('B', 2); // Lift B starting at floor 2
 */
...
/**
 * Processes one simulation tick for a single elevator.
 * This is the core elevator logic that handles:
 * - Sensor readings and safety system updates
 * - Cable snap emergency procedures
 * - Power outage and battery mode
 * - Door operations (opening, open, closing)
 * - Movement and dispatch logic (SCAN algorithm)
 * - Passenger boarding/alighting
 * - Hazard zone approval requests
 * 
 * @param lift - Current state of this elevator
 * @param otherLift - State of the other elevator (for dispatch coordination)
 * @param buildingState - Current building state with waiting passengers per floor
 * @param currentStats - Current simulation statistics
 * @param systemMode - Current system mode (NORMAL, FIRE_ALARM, etc.)
 * @param fireFloor - Floor number where fire is detected, or null
 * @param mainsSpeedMultiplier - Speed multiplier from mains power (0-1)
 * @param hasMainsPower - Whether mains power is available
 * @param isCableSnapped - Whether cable snap is triggered for this lift
 * @param snapTimerRef - Reference to cable snap evacuation timer
 * @param approvalRequests - Current pending approval requests
 * @param simTime - Current simulation time in seconds
 * @param timeScale - Time scale multiplier for simulation speed
 * @returns TickResult containing updated states and generated logs
 */
