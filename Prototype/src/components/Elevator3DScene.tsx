// @ts-nocheck
/**
 * 3D Elevator Scene Component
 * 
 * Renders a full 3D visualization of the elevator system using React Three Fiber.
 * Features:
 * - Gearless traction machine with integrated sheave
 * - Dual elevator shafts (Lift A & B)
 * - WASD camera controls with scroll zoom
 * - Real-time animation synced with simulation state
 * 
 * @module components/Elevator3DScene
 */
...
const GearlessTractionMachine = ({ velocity, xOffset }: { velocity: number; xOffset: number }) => {
  ...
  // Cleanup: dispose of cloned scene geometries and materials on unmount
  useEffect(() => {
    return () => {
...
// --- PROCEDURAL CABIN ---
// Simple procedural elevator cabin (replaces FBX model for reliability)
const ElevatorCabin = ({ position, doorsOpen, xOffset, liftId }: {
  ...
  const cabinColor = liftId === 'A' ? "#3b82f6" : "#10b981";
...
const ElevatorShaft = ({ lift, xOffset }: { lift: LiftState; xOffset: number }) => {
  // Use currentFloor for position to sync with 2D model
  const floorPosition = lift.currentFloor - 1; // 0-indexed for 3D (floors 1-5 become 0-4)
  // Use doorOpenProgress for smooth door animation (0-1)
  const doorsOpen = lift.doorOpenProgress > 0.5;
