...
import { LiftState, BuildingState, Passenger } from '../types';
import { FLOORS, FLOOR_PIXELS, LIFT_CAPACITY_KG } from '../constants';
import { ArrowUp, ArrowDown, User, Flame, ZapOff, Cross } from 'lucide-react';

interface Props {
  liftA: LiftState;
  liftB: LiftState;
  building: BuildingState;
  mode: 'NORMAL' | 'FIRE_ALARM' | 'POWER_OUTAGE';
}
...
const PassengerIcon: React.FC<{ p: Passenger; insideLift: boolean }> = ({ p, insideLift }) => (
  ...
  >
    ...
    />
    {/* Mini Badge for weight */}
    <div className="absolute -top-1 -right-1 w-2 h-2 rounded-full bg-blue-500" />
  </div>
);

const LiftShaft: React.FC<{ lift: LiftState; name: string; mode: string }> = ({ lift, name, mode }) => {
  ...
  return (
    <div className="relative w-40 h-[480px] bg-slate-800/80 border-x border-slate-700/50 flex flex-col justify-end backdrop-blur-sm shadow-inner">
      ...
      <div 
        className={`absolute left-3 right-3 h-[140px] rounded-lg shadow-xl flex flex-col overflow-hidden transition-all duration-100 ease-linear
          ${mode === 'POWER_OUTAGE' ? 'bg-slate-900 border border-slate-700 grayscale' : 'bg-slate-200 border-2 border-slate-400'}
      ...
      >
        {/* LED Header Display */}
        <div className="bg-slate-900 px-2 py-1 flex justify-between items-center border-b-2 border-slate-800 h-[28px]">
            <span className="text-[10px] text-slate-500 font-mono tracking-tighter">{name}</span>
            <div className="flex items-center gap-2">
                {/* Direction Arrows */}
                {lift.direction === 'UP' && <ArrowUp size={12} className="text-green-500 animate-pulse" />}
                {lift.direction === 'DOWN' && <ArrowDown size={12} className="text-red-500 animate-pulse" />}
                
                {/* Floor Number Display */}
                <div className="bg-black text-red-500 font-mono font-bold text-xs px-1 rounded border border-slate-700 shadow-[0_0_5px_rgba(239,68,68,0.5)]">
                    {Math.round(lift.currentFloor)}
                </div>
            </div>
        </div>
        ...
        <div className="h-[6px] bg-slate-800 w-full relative">
            <div 
                className={`h-full transition-all duration-300 ${loadColor}`} 
...
export const BuildingVisualizer: React.FC<Props> = ({ liftA, liftB, building, mode }) => {
  return (
    <div className="bg-slate-950 p-6 rounded-xl border border-slate-700 shadow-2xl relative overflow-hidden">
        {mode === 'FIRE_ALARM' && (
      ...
      <div className="flex justify-center gap-12 relative z-10">
        ...
        <div className="flex flex-col-reverse justify-between h-[480px] py-[70px] mr-auto relative z-10 w-48">
          {FLOORS.map(f => {
            // Check if any lift is targeting this floor
            ...
            return (
                ...
                <div className="text-right w-full pr-4 border-r border-slate-700 relative">
                    ...
                    <div className={`text-3xl font-bold transition-colors duration-300 ${waitingCount > 0 ? 'text-white' : 'text-slate-600'}`}>
                        L{f}
                    </div>
        ...
        {/* Lift A */}
        <LiftShaft lift={liftA} name="LIFT A" mode={mode} />
        
        {/* Central Gap/Pillar */}
        <div className="w-12 bg-slate-900 border-x border-slate-800 h-[480px] opacity-50" />
        
        {/* Lift B */}
        <LiftShaft lift={liftB} name="LIFT B" mode={mode} />

      </div>
      
      <div className="mt-6 flex justify-between items-center text-xs font-mono border-t border-slate-800 pt-4">
        <div className="flex gap-4">
            ...
            <div className="flex items-center gap-2"><div className="w-2 h-2 bg-red-500 rounded-full"/> Overload</div>
        </div>
        <div>Mode Sistem: <span className={`font-bold ${mode === 'NORMAL' ? 'text-green-400' : 'text-red-400'}`}>{mode}</span></div>
      </div>
    </div>
  );
};
