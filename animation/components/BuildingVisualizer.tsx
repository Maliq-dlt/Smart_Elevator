...
import { ArrowUp, ArrowDown, User, Flame, ZapOff, Cross, Waves, Activity, AlertOctagon, Battery } from 'lucide-react';
      ...
      >
        ...
        {isMaintenance && (
          <div className="absolute inset-0 z-30 bg-black/80 flex flex-col items-center justify-center text-center">
            ...
            <span className="text-red-500 font-bold font-mono text-sm border-2 border-red-500 px-2 py-1 rotate-[-10deg]">OUT OF<br />ORDER</span>
          </div>
        )}
...
export const BuildingVisualizer: React.FC<Props> = ({ liftA, liftB, building, mode, fireFloor }) => {
  return (
    <div className={`bg-slate-950 p-6 rounded-xl border border-slate-700 shadow-2xl relative overflow-hidden transition-transform duration-100 ${mode === 'EARTHQUAKE' ? 'animate-[shake_0.5s_ease-in-out_infinite]' : ''}`}>
      ...
      <div className="mt-6 flex justify-between items-center text-xs font-mono border-t border-slate-800 pt-4 relative z-20 bg-slate-950">
        ...
        <div>Skenario: <span className={`font-bold uppercase ${mode === 'NORMAL' ? 'text-green-400' : 'text-red-400'}`}>{mode.replace('_', ' ')}</span></div>
      </div>
