...
export default function App() {
    ...
    useEffect(() => {
        ...
        const interval = setInterval(() => {
            ...
            const newPax: PassengerConfig = {
                ...
                startFloor: start, destinationFloor: dest, requestTime: simTime + 0.5
            };
            setPendingPassengers(prev => [...prev, newPax]);
            addLog(`AUTO SIM: ${newPax.name} generated at Floor ${start}.`, 'INFO');
        }, 2000); // Generate passenger every 2 seconds
    ...
    const updateSystem = () => {
        ...
        instantaneousJ += (200 * powerFactor); // Base/idle consumption

        // Accumulate total energy consumed
        nextStats.totalEnergyJ += instantaneousJ * (TICK_RATE / 1000);

        // Update energy history graph every second
        ...
        // --- Update Floor Visits ---
        // Track when lifts stop at a floor (transition to DOOR_OPENING)
        const finalStats = { ...resultB.stats };

        if (resultA.lift.status === 'DOOR_OPENING' && liftA.status !== 'DOOR_OPENING') {
            const floor = Math.round(resultA.lift.currentFloor);
            finalStats.floorVisits[floor] = (finalStats.floorVisits[floor] || 0) + 1;
        }
        if (resultB.lift.status === 'DOOR_OPENING' && liftB.status !== 'DOOR_OPENING') {
            const floor = Math.round(resultB.lift.currentFloor);
            finalStats.floorVisits[floor] = (finalStats.floorVisits[floor] || 0) + 1;
        }

        // --- Update Passenger Load Stats ---
        const totalCurrentPassengers = resultA.lift.passengers.length + resultB.lift.passengers.length;
        if (totalCurrentPassengers > finalStats.peakPassengers) {
            finalStats.peakPassengers = totalCurrentPassengers;
        }
        // Only track min after at least one passenger has been picked up
        if (finalStats.peakPassengers > 0 && (finalStats.minPassengers === 0 || totalCurrentPassengers < finalStats.minPassengers)) {
            finalStats.minPassengers = totalCurrentPassengers;
        }
